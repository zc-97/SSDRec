...
class NARM(SequentialRecommender):
    ...
    def forward_denoising(self, item_seq, item_seq_len, item_seq_emb):

        item_seq_emb_dropout = self.emb_dropout(item_seq_emb)
        gru_out, _ = self.gru(item_seq_emb_dropout)

        generated_item_seq_len = item_seq_len - 1
        # 将index为-1 的项置为0，防止数组越下界
        generated_item_seq_len = generated_item_seq_len * generated_item_seq_len.gt(0)


        # fetch the last hidden state of last timestamp
        c_global = ht = self.gather_indexes(gru_out, generated_item_seq_len)
        # avoid the influence of padding
        mask = item_seq.gt(0).unsqueeze(2).expand_as(gru_out)
        q1 = self.a_1(gru_out)
        q2 = self.a_2(ht)

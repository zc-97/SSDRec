...
class BERT4Rec(SequentialRecommender):
    ...
    def reconstruct_train_data_denoising(self, item_seq):
        """
        Mask item sequence for training.
        1.不mask id 为0 的item
        2.返回的mask index没有padding
        """
        device = item_seq.device
        batch_size = item_seq.size(0)

        sequence_instances = item_seq.detach().cpu().numpy().tolist()
    ...
    def forward_denoising(self, item_seq, item_seq_emb):
        # masked_item_seq, pos_items, neg_items, masked_index = self.reconstruct_train_data_denoising(item_seq)
        # for i in item_seq.size(1):
        #     item_seq_emb[:, masked_index[i], :] = self.item_embedding(torch.tensor([self.mask_token], device=self.device))

        position_ids = torch.arange(item_seq.size(1), dtype=torch.long, device=item_seq.device)
        position_ids = position_ids.unsqueeze(0).expand_as(item_seq)
        position_embedding = self.position_embedding(position_ids)
        position_embedding = position_embedding * item_seq.gt(0).unsqueeze(2)
